...
Fix: instead of training the model to predict the raw passenger count, train it to predict growth relative to the same calendar month last year — `(actual - same_month_last_year) / same_month_last_year` — then reconstruct the forecast as `same_month_last_year × (1 + predicted_growth)`. A growth ratio stays in a roughly stable range even while the underlying level keeps climbing (unlike the raw count), so the tree no longer needs to extrapolate past its training range to produce a new all-time high — it just needs "stronger or weaker than a year ago," which it can express regardless of what absolute levels it saw in training. (Also fixed a division-by-zero the growth-ratio target introduced: BFS's genuine zero-passenger months, Apr/May 2020, make a same-month-last-year base of 0 for any target one year later — skipped those training examples, and fall back to the last known level at prediction time rather than dividing by zero.)

COVID-excluded, v1 vs v2:

| Airport | ets MAE | xgb v1 MAE | xgb v2 MAE | v1 vs v2 |
|---|---|---|---|---|
| BFS | **32,946.65** | 65,655.08 | 66,949.11 | +2.0% (worse) |
| BHD | **11,436.26** | 14,718.48 | 15,939.51 | +8.3% (worse) |
| LDY | **1,784.05** | 2,966.12 | 3,030.10 | +2.2% (worse) |

**The fix didn't work — worth stating plainly rather than burying it.** Retargeting on year-over-year growth made every airport's MAE and MAPE slightly worse, not better. The one partial exception: BFS's RMSE improved (81,668 → 77,975), consistent with fewer of the very large under-forecast misses the extrapolation-capping theory predicts — but that didn't translate into a better MAE or MAPE, and BHD and LDY got uniformly worse on every metric. Full-data (COVID-included) results regressed more sharply (e.g. BFS MAE 98,162 → 152,599), likely because the zero-baseline training examples that were skipped were disproportionately clustered right around the COVID period, meaning the model has less to learn from exactly where it needs it most.

**Revised conclusion**: the trend-extrapolation ceiling was a real, defensible hypothesis and likely still a genuine contributing factor, but the growth-ratio fix targeted at it didn't close the gap with ETS — either it isn't the dominant cause of the underperformance, or it's offset by a cost the reformulation introduces (a ratio is noisier than a level for a low-volume, high-relative-variance series like LDY, and the skipped zero-baseline examples reduce the training set right at the most informative, highest-variance months). Didn't chase this further given the size of the remaining gap and that ETS already gives a clean, unambiguous win at every airport. The more likely explanation at this point: with ~130 monthly observations per airport, there simply isn't enough data for a general-purpose gradient-boosted model to out-learn a classical model (ETS) that's already structurally well-matched to the problem (explicit trend + seasonal decomposition, recursive state updates) — a legitimate and fairly common real-world finding, not a failure of the exercise.

**Decision**: ETS is the model reported as this project's best per-airport forecaster. XGBoost is documented as an instructive negative result — attempted, debugged, improved on its own terms, but never closed the gap with a well-specified classical model on this amount of data.
